...
- Sistema de check-in: terminado (2026-07-02). Búsqueda por nombre/código en el Panel de Anfitrión, marcado de entrada vía Edge Function `hacer-checkin` (valida dueño del evento, `estado = "activo"`, y bloquea doble check-in). La tabla `boletos` no tenía la columna `checkin_en` — se agregó en esta sesión.
- De paso se corrigió un hueco de seguridad relacionado: la activación de boletos de pago confiaba en un parámetro de la URL de retorno de Mercado Pago (`collection_status`) en vez de verificar el pago real. Ahora `PagoExitoso.jsx` llama a la Edge Function `confirmar-pago-mp`, que verifica el pago contra la API de Mercado Pago. También se eliminaron las políticas RLS de `UPDATE` en `boletos` que permitían a cualquier usuario escribir sus propias columnas `estado`/`checkin_en` directo (ya no se necesitan: todas las escrituras a `boletos` pasan por Edge Functions con `service_role`).
- Pendiente de probar: una compra de boleto de pago real de punta a punta con `confirmar-pago-mp` (se intentó pero Mercado Pago bloqueó el pago por usar una tarjeta vinculada a la misma identidad del host de prueba — hace falta una tarjeta de otra persona para probarlo).
