...
useEffect(() => {
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#080808", color: "white", fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif" }}>
      ...
      {/* HEADER */}
      <div style={{ background: "linear-gradient(135deg, #0d0b2e 0%, #120f3a 50%, #080808 100%)", padding: "56px 64px 48px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
          <motion.h1 initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} style={{ fontSize: "2.2rem", fontWeight: 700, letterSpacing: "-0.8px", marginBottom: "8px" }}>
            Explorar eventos
          </motion.h1>
          <p style={{ color: "rgba(255,255,255,0.4)", fontSize: "15px", marginBottom: "32px" }}>Encuentra tu próxima experiencia</p>

          {/* BARRA DE BÚSQUEDA */}
          <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
            <div style={{ display: "flex", alignItems: "center", flex: 1, minWidth: "280px", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "12px", padding: "0 16px", gap: "10px" }}>
              ...
              />
              {busqueda && (
                <button onClick={() => setBusqueda("")} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.3)", cursor: "pointer", fontSize: "18px", lineHeight: 1 }}>×</button>
              )}
            </div>
            <select value={orden} onChange={e => setOrden(e.target.value)}
              style={{ background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "12px", color: "white", padding: "0 16px", fontSize: "14px", cursor: "pointer", outline: "none", fontFamily: "inherit", minWidth: "160px", colorScheme: "dark" }}
            >
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "40px 64px" }}>
        ...
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "36px" }}>
          {categorias.map(cat => (
            <motion.button key={cat} onClick={() => setCategoria(cat)} whileTap={{ scale: 0.95 }}
              style={{ padding: "8px 18px", borderRadius: "999px", cursor: "pointer", border: `1px solid ${categoria === cat ? "rgba(124,58,237,0.6)" : "rgba(255,255,255,0.1)"}`, background: categoria === cat ? "rgba(124,58,237,0.2)" : "transparent", color: categoria === cat ? "white" : "rgba(255,255,255,0.5)", fontSize: "13.5px", fontWeight: categoria === cat ? 600 : 400, fontFamily: "inherit", transition: "all 0.15s" }}
            >{cat}</motion.button>
          ))}
        </div>

        {/* RESULTADOS */}
        <div style={{ marginBottom: "20px", color: "rgba(255,255,255,0.35)", fontSize: "13.5px" }}>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: "center", padding: "80px 24px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "20px" }}>
            ...
            <div style={{ fontWeight: 600, fontSize: "18px", marginBottom: "8px" }}>No encontramos eventos</div>
            ...
            <button onClick={() => { setBusqueda(""); setCategoria("Todas") }}
              style={{ background: "rgba(124,58,237,0.2)", border: "1px solid rgba(124,58,237,0.4)", borderRadius: "10px", color: "#a78bfa", padding: "10px 24px", fontWeight: 600, fontSize: "14px", cursor: "pointer", fontFamily: "inherit" }}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "18px" }}>
            {eventosFiltrados.map((ev, i) => {
              const fecha = new Date(ev.fecha)
              const fechaFormato = fecha.toLocaleDateString("es-MX", { weekday: "short", day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })
              return (
                <motion.div key={ev.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
                  style={{ borderRadius: "20px", overflow: "hidden", background: "#0f0f11", border: "1px solid rgba(255,255,255,0.07)", cursor: "pointer", transition: "border 0.2s, transform 0.2s" }}
                  whileHover={{ y: -4, borderColor: "rgba(255,255,255,0.15)" }}
                  onClick={() => window.location.href = `/evento/${ev.id}`}
                >
                  <div style={{ position: "relative", height: "180px", overflow: "hidden" }}>
                    <motion.img src={ev.imagen_url || "https://images.unsplash.com/photo-1516450360452-9312f5e86fc7?w=500&q=80"} alt={ev.titulo}
                      whileHover={{ scale: 1.05 }} transition={{ duration: 0.4 }}
                      style={{ width: "100%", height: "100%", objectFit: "cover" }}
                    />
                    <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to bottom, rgba(0,0,0,0.05), rgba(0,0,0,0.7))" }} />
                    <span style={{ position: "absolute", top: "12px", left: "12px", background: "rgba(0,0,0,0.6)", backdropFilter: "blur(12px)", borderRadius: "999px", padding: "4px 12px", fontSize: "12px", fontWeight: 600 }}>{ev.categoria}</span>
                    <span style={{ position: "absolute", top: "12px", right: "12px", background: ev.tipo_boleto === "instantaneo" ? "rgba(124,58,237,0.7)" : "rgba(37,99,235,0.7)", backdropFilter: "blur(12px)", borderRadius: "999px", padding: "4px 12px", fontSize: "12px", fontWeight: 600 }}>
                      {ev.tipo_boleto === "instantaneo" ? "⚡ Instantáneo" : "📋 Solicitud"}
                    </span>
                  </div>
                  <div style={{ padding: "18px 20px" }}>
                    <div style={{ fontWeight: 700, fontSize: "16px", marginBottom: "3px", letterSpacing: "-0.2px" }}>{ev.titulo}</div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      <div>
                        <span style={{ fontWeight: 700, fontSize: "20px", letterSpacing: "-0.5px" }}>{ev.precio === 0 ? "Gratis" : `$${ev.precio}`}</span>
                        {ev.precio > 0 && <span style={{ fontSize: "12px", color: "rgba(255,255,255,0.35)", marginLeft: "4px" }}>MXN</span>}
                      </div>
                      <span style={{ fontSize: "13px", color: "#a78bfa", fontWeight: 500 }}>Ver detalles →</span>
                    </div>
                  </div>
                </motion.div>
              )
