...
const categoryIcons = {
  ...
  "Autos": <svg width="32" height="32" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path d="M19 17h2c.6 0 1-.4 1-1v-3c0-.9-.7-1.7-1.5-1.9C18.7 10.6 16 10 16 10s-1.3-1.4-2.2-2.3c-.5-.4-1.1-.7-1.8-.7H5c-.6 0-1.1.4-1.4.9l-1.4 2.9A3.7 3.7 0 0 0 2 12v4c0 .6.4 1 1 1h2"/><circle cx="7" cy="17" r="2"/><path d="M9 17h6"/><circle cx="17" cy="17" r="2"/></svg>,
  ...
  "Deportes": <svg width="32" height="32" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24"><path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6"/><path d="M18 9h1.5a2.5 2.5 0 0 0 0-5H18"/><path d="M4 22h16"/><path d="M10 14.66V17c0 .55-.47.98-.97 1.21C7.85 18.75 7 20.24 7 22"/><path d="M14 14.66V17c0 .55.47.98.97 1.21C16.15 18.75 17 20.24 17 22"/><path d="M18 2H6v7a6 6 0 0 0 12 0V2Z"/></svg>,
}
...
function HomePage({ user, perfil, onLogout, setFotoZoom }) {
  ...
  return (
    <div style={{ minHeight: "100vh", color: "white", fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", overflowX: "hidden" }}>
      ...
      {/* ── HERO ── */}
      {/* Sin fondo propio: el video se funde directo con el fondo normal de
          la página (una sola transición, la del video difuminado) — darle a
      ...
      <section style={{ position: "relative", overflow: "hidden" }}>
        ...
        {isMobile ? (
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.3 }}
            style={{ display: "flex", alignItems: "center", gap: "8px", maxWidth: "760px", margin: "0 auto", padding: "8px", borderRadius: "16px", background: "rgba(19,17,26,0.92)", border: "1px solid rgba(255,255,255,0.09)", boxShadow: "0 20px 50px -22px rgba(0,0,0,0.8)" }}
          >
            <div style={{ flex: 1, display: "flex", alignItems: "center", gap: "10px", padding: "12px 15px", borderRadius: "10px", background: "rgba(255,255,255,0.045)" }}>
              <svg width="16" height="16" fill="none" stroke="rgba(231,228,245,0.45)" strokeWidth="2" viewBox="0 0 24 24"><circle cx="11" cy="11" r="7"/><path d="m20 20-3.5-3.5"/></svg>
              <input value={busquedaHero} onChange={e => setBusquedaHero(e.target.value)} onKeyDown={e => e.key === "Enter" && navigate(`/explorar?q=${encodeURIComponent(busquedaHero)}&estado=${encodeURIComponent(estadoHero)}&fecha=${encodeURIComponent(fechaHero)}`)} placeholder="Buscar eventos..." style={{ flex: 1, background: "transparent", border: "none", outline: "none", color: "white", fontSize: "14px", padding: 0, fontFamily: "inherit" }} />
            </div>
            <span style={{ width: "1px", height: "24px", background: "rgba(255,255,255,0.1)", flexShrink: 0 }} />
            <div style={{ display: "flex", alignItems: "center", gap: "8px", padding: "0 8px" }}>
              <svg width="15" height="15" fill="none" stroke="#8b6bff" strokeWidth="2" viewBox="0 0 24 24"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
              <select value={estadoHero} onChange={e => setEstadoHero(e.target.value)}
                style={{ background: "transparent", border: "none", outline: "none", color: estadoHero ? "white" : "rgba(231,228,245,0.85)", fontSize: "14px", fontWeight: 500, cursor: "pointer", fontFamily: "inherit", width: "128px" }}
              >
                <option value="" style={{ background: "#111" }}>Todo México</option>
                {["Aguascalientes","Baja California","Baja California Sur","Campeche","Chiapas","Chihuahua","Ciudad de México","Coahuila","Colima","Durango","Estado de México","Guanajuato","Guerrero","Hidalgo","Jalisco","Michoacán","Morelos","Nayarit","Nuevo León","Oaxaca","Puebla","Querétaro","Quintana Roo","San Luis Potosí","Sinaloa","Sonora","Tabasco","Tamaulipas","Tlaxcala","Veracruz","Yucatán","Zacatecas"].map(e => (
                  <option key={e} value={e} style={{ background: "#111" }}>{e}</option>
            ...
            <span style={{ width: "1px", height: "24px", background: "rgba(255,255,255,0.1)", flexShrink: 0 }} />
            <div style={{ display: "flex", alignItems: "center", gap: "8px", padding: "0 8px" }}>
              <svg width="15" height="15" fill="none" stroke="#8b6bff" strokeWidth="2" viewBox="0 0 24 24"><rect x="4" y="5" width="16" height="16" rx="2"/><path d="M4 9h16M9 3v4M15 3v4"/></svg>
              <select value={fechaHero} onChange={e => setFechaHero(e.target.value)}
                style={{ background: "transparent", border: "none", outline: "none", color: fechaHero ? "white" : "rgba(231,228,245,0.85)", fontSize: "14px", fontWeight: 500, cursor: "pointer", fontFamily: "inherit", width: "138px" }}
              >
            ...
            <motion.button whileHover={{ opacity: 0.9 }} whileTap={{ scale: 0.97 }}
              style={{ border: "none", padding: "13px 26px", borderRadius: "10px", background: "linear-gradient(140deg, #8b6bff, #6d3ff0)", color: "white", fontSize: "14px", fontWeight: 700, cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap", boxShadow: "0 8px 22px -8px rgba(124,92,255,0.8)", flexShrink: 0 }}
              onClick={() => navigate(`/explorar?q=${encodeURIComponent(busquedaHero)}&estado=${encodeURIComponent(estadoHero)}&fecha=${encodeURIComponent(fechaHero)}`)}
            >Buscar</motion.button>
          </motion.div>
        )}
